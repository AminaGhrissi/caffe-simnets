...
void DeconvolutionLayer<Dtype>::compute_output_shape() {
  ...
  bool round_up = this->layer_param_.convolution_param().round_up_output_dims();
  for (int i = 0; i < this->num_spatial_axes_; ++i) {
    // i + 1 to skip channel axis
    const int input_dim = this->input_shape(i + 1);
    const int kernel_extent = dilation_data[i] * (kernel_shape_data[i] - 1) + 1;
    int round_down_dim = stride_data[i] * (input_dim - 1)
        + kernel_extent - 2 * pad_data[i];
    const int output_dim =  round_up ?  round_down_dim + 1 : round_down_dim ;
    // if (round_up) {
    //   output_dim = stride_data[i] * (input_dim - 1)
    //     + kernel_extent - 2 * pad_data[i] + 1;
    // }
