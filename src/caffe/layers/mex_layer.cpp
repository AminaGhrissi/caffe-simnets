...
void MEXLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
  MEXParameter mex_param = this->layer_param_.mex_param();
  BlockParameter block_param = mex_param.block_param();
  expects_labels_ = false;
  maximum_entropy_regularization_coeff_ = mex_param.maximum_entropy_regularization_coeff();
  normalize_patches_ = mex_param.normalize_patches();
  if (normalize_patches_) {
    normalization_fudge_factor_ = mex_param.normalization_fudge_factor();
    normalize_variance_ = mex_param.normalize_variance();
  }
  softmax_mode_ = mex_param.softmax_mode();

  // Configure input size and number of instances
  channels_ = bottom[0]->channels();
  height_ = bottom[0]->height();
  width_ = bottom[0]->width();
  num_instances_ = mex_param.num_instances();
  ...
  blocks_out_of_bounds_value_ = block_param.out_of_bounds_value();
  blocks_round_down_ = block_param.round_down();
  height_out_ = dimension_out_size(height_, pad_h_, block_h_, stride_h_, blocks_round_down_);
  width_out_ = dimension_out_size(width_, pad_w_, block_w_, stride_w_, blocks_round_down_);
  channels_out_ = dimension_out_size(channels_, pad_c_, block_c_, stride_c_, blocks_round_down_);
